...
module.exports = function (context) {
  ...
  var xwalkVariables = {},
    ...
    androidPlatformDir = path.join(
      context.opts.projectRoot,
      "platforms",
      "android"
    ),
    projectConfigurationFile = path.join(
      context.opts.projectRoot,
      "config.xml"
    ),
  ...
  var oldConfigXMLLocation = path.join(
    androidPlatformDir,
    "res",
    "xml",
    "config.xml"
  );
  var newConfigXMLLocation = path.join(
    androidPlatformDir,
    "app",
    "src",
    "main",
  ...
  var addPermission = function () {
    var projectManifestXmlRoot = XmlHelpers.parseElementtreeSync(
      projectManifestFile
    );
    var child = et.XML(
    ...
    fs.writeFileSync(
      projectManifestFile,
      projectManifestXmlRoot.write({ indent: 4 }),
      "utf-8"
    );
  };
  ...
  var defaultPreferences = function () {
    ...
    var pluginXmlRoot = XmlHelpers.parseElementtreeSync(
        pluginConfigurationFile
      ),
      tagName = "preference",
      containerName = "config-file",
      targetPlatform = "android",
      targetPlatformTag = pluginXmlRoot.find(
        './platform[@name="' + targetPlatform + '"]'
      );

    var tagsInRoot = pluginXmlRoot.findall(tagName) || [],
      tagsInPlatform = targetPlatformTag
        ? targetPlatformTag.findall(tagName)
        : [],
      tagsInContainer = targetPlatformTag
        ? targetPlatformTag.findall(containerName)
        : [],
      ...
      prefTag.getchildren().forEach(function (element) {
        if (
          ...
          if (element.attrib["name"] == "xwalkLiteVersion") {
            ...
            pluginPreferences[element.attrib["name"]] =
              element.attrib["default"];
          }
        }
      });
  ...
  this.beforeBuild64bit = function () {
    if (build64bit()) {
      var configXmlRoot = XmlHelpers.parseElementtreeSync(
        projectConfigurationFile
      );
      var child = configXmlRoot.find(
        './preference[@name="' + xwalk64bit + '"]'
      );
      if (!child) {
        child = et.XML(
